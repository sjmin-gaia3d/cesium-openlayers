...
const useActivateCesiumSelect = ({ dataSource, activeInteraction, callback }) => {
    ...
    const handleMouseLeftClick = (eventViewer, event) => {
        if (!dataSource || activeInteraction!==ACTIVE_INTERACTIVE_TYPES.SELECT) return;
        ...
        if (pickedObject && Cesium.defined(pickedObject.id)) {
            ...
            callback?.(convertEntityToStoreSelectedObject(pickedObject.id, ACTIVE_INTERACTIVE_TYPES.SELECT))
        }
    };
...
export default useActivateCesiumSelect;
